...
from .owner import OwnerDetailView, OwnerListView, OwnerDeleteView
import os
import pandas as pd
import sys
import csv
...
class PDetailView(OwnerDetailView):
    template_name = 'pfds/pfd_detail.html'
    def get(self,request, pk):
        ppatient = PPatient.objects.get(id = pk)
        fvc = FVC.objects.filter(ppatient_id= pk)
        ctx  = {'fvc_list': fvc,'ppatient':ppatient }
...
def chart(request,pk):
    ...
    dataSource['data'] =[]
    for key in FVC.objects.filter(ppatient_id = pk):
        data = {}
        data['label'] = key.week
        data['value'] = key.fvc_value_final
        dataSource['data'].append(data)
    spline = FusionCharts("spline", "ex1", 700, 400, "chart-1", "json",dataSource)
    #column2D = FusionCharts("column2D", "ex1" , "600", "350", "chart-1", "json", dataSource)
    #ctx  = {'fvc_list': fvc,'ppatient':ppatient }
    return render(request, 'pfds/fusionchart.html',{'output':spline.render(),'fvc_list': fvc,'ppatient':ppatient })
    #{'output':column2D.render(),'fvc_list': fvc,'ppatient':ppatient }
class PDeleteView(OwnerDeleteView):
    model = PPatient
    template_name = 'pfds/pfd_confirm_delete.html'
    success_url = reverse_lazy('pfds:all')
    def get(self, request, pk):
        ppatient = get_object_or_404(self.model, pk=pk)
        ctx = {'ppatient': ppatient}
...
class PListView(OwnerListView):
    template_name = 'pfds/pfd_list.html'
    def get(self,request):
        model = PPatient
        ppatient_list = PPatient.objects.all()
        ctx ={'ppatient_list': ppatient_list}
        return render(request,self.template_name,ctx)
...
def Multipleupload_save(request):
    ...
    a = float(np.quantile(_a, 0.5))
    
    ppatient1.relative_a = a
    #ppatient1.relative_a = -7.974595069885254
    ppatient1.relative_b = float(ppatient1.FVC_base) - a*float(ppatient1.week_base)
    #ppatient1.relative_b = 3381.390281677246
    #-------------------------------------------------------------
    data =[]
    #demo = pd.read_csv("demo.csv")
    for i in range(int(week_start),int(week_end)+1):
    ...
    df1 = img_sub   #.reset_index(drop=True)   .sort_values(by=['Patient_Week'], ascending=True)
    df2 = reg_sub   #.reset_index(drop=True)   .sort_values(by=['Patient_Week'], ascending=True)
    ...
    with open(os.path.join(base_dir,'efficient_sub.csv'),mode='r', **kwargs) as fp:
        ...
        for row in reader:
            if line_count ==0:
                ...
                line_count+=1
                week = row[0].split('_')[-1]
                fvc = row[1]
                conf = row[2]
                q = FVC(ppatient_id=ppatient1,week= week,fvc_value_final = fvc, confidence = conf)
                q.save()

        #pimage = PPatientImages(ppatient_id= ppatient1, image=file_path)
        #pimage.save()
    #fvc = FVC.objects.filter(ppatient_id= ppatient1.id)
    #ctx  = {'fvc_list': fvc,'ppatient':ppatient1 }
    #success_template = 'pfds/pfd_detail.html'
    return chart(request,ppatient1.id)
    #return render(request, success_template, ctx)
